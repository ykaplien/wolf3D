...
void	floor_ceiling_calc(t_wolf *wolf, int ver_pix, int x)
{
	...
	current_dist = WIN_Y / (WIN_Y - (ver_pix * 2.0));
	current_floor_pos[0] = current_dist * wolf->floor_pos_y + (1.0 - (ver_pix / wolf->perp_wall_dist)) * wolf->pos_x;
	current_floor_pos[1] = current_dist * wolf->floor_pos_x + (1.0 - (ver_pix / wolf->perp_wall_dist)) * wolf->pos_y;
...
void	put_floor(t_wolf *wolf, int x, int y)
{
	...
	wolf->image[pixels] = wolf->text[0].addr[4 * (64 * wolf->floor_t_y + wolf->floor_t_x)];
	wolf->image[pixels + 1] = wolf->text[0].addr[4 * (64 * wolf->floor_t_y + wolf->floor_t_x) + 1];
	wolf->image[pixels + 2] = wolf->text[0].addr[4 * (64 * wolf->floor_t_y + wolf->floor_t_x) + 2];
}
...
void	put_ceiling(t_wolf *wolf, int x, int y)
{
	...
	pixels = (x * 4) + ((WIN_X * 4) * y);
	wolf->image[pixels] = wolf->text[1].addr[4 * (64 * wolf->floor_t_y + wolf->floor_t_x)];
	wolf->image[pixels + 1] = wolf->text[1].addr[4 * (64 * wolf->floor_t_y + wolf->floor_t_x) + 1];
	wolf->image[pixels + 2] = wolf->text[1].addr[4 * (64 * wolf->floor_t_y + wolf->floor_t_x) + 2];
}
